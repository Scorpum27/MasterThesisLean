...
public class VisualizerPedigreeVCx {
	...
	// %%% --- %% --- %% --- %% --- %% --- %% --- %% --- %% --- DISPLAY EVOLUTION OF NETWORKS---%%---%%---%%---%%---%%---%%---%%---%%---%%---%%%
		...
		bgImage = ImageIO.read(new FileInputStream(simName + "/zurich_1pm/bgImgVCfacilities3crop.png"));
		...
		final Double x0 = -200.0; // top left corner of background image
		final Double y0 = 8200.0; // top left corner of background image
		final Double x1 = 8200.0; // bottom right corner of background image
		final Double y1 = -200.0; // bottom right corner of background image
...
//					"CAUTION: Pedigree tree has not same number of generations as finalGeneration! Please check.");
		...
		for (Integer n=finalNetworkNrFirst; n<=finalNetworkNrLast; n++) {
			...
			for (Integer gen = finalGeneration; gen >= firstGeneration; gen--) {
				...
				blankImg = ImageIO.read(new File(simName + "/zurich_1pm/bgImgVCfacilities3crop.png"));
